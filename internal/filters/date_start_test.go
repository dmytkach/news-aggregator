...
func TestDateStart_Filter(t *testing.T) {
	tests := []struct {
		name      string
		startDate time.Time
		news      []entity.News
		...
		{
			name:      "should filter news from a given date.",
			startDate: time.Date(2024, time.May, 18, 23, 0, 0, 0, time.UTC),
			news: []entity.News{
				{
		...
		{
			name:      "should return empty slice if incorrect date.",
			startDate: time.Date(2025, time.May, 18, 23, 0, 0, 0, time.UTC),
			news: []entity.News{
				{
					Title:       "Container ship that struck Baltimore bridge will be removed from the site 'within days,' Maryland governor says",
					Description: "Ship that struck Francis Scott Key Bridge in Baltimore will be removed \"within days,\" Maryland Gov. Wes Moore says",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := dsf.Filter(tt.news); !reflect.DeepEqual(got, tt.want) {
