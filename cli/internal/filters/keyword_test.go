...
func TestKeyword_Filter(t *testing.T) {
	tests := []struct {
		name     string
		keywords []string
		news     []entity.News
		...
		{
			name:     "Should filter by the specified parameters in the title of news.",
			keywords: []string{"American"},
			news: []entity.News{
				{
		...
		{
			name:     "Should filter by the specified parameters in the description of news.",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := kf.Filter(tt.news); !reflect.DeepEqual(got, tt.want) {
